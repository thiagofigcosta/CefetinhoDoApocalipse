...
void GL::drawPause(){
    ...
    for(int i=0;i<text.size();i+=maxChars){
        ...
        GL::getTextureByName("background1"),1);
        GL::drawTexture((Util::nTRectangleSet(
...
void GL::drawHUD(){
    ...
    point.setPoint(Scenes::camera.x.movedCam+250,Scenes::camera.y.movedCam+23,1);
    char buffer[5];
    snprintf(buffer,5,"%d",Player::lives);
    string strL(buffer);
    GL::drawText(point,(char*)("Vidas: "+strL).c_str(),GL::getColorByName("black"));
    point.setPoint(Scenes::camera.x.movedCam+360,Scenes::camera.y.movedCam+23,1);
    snprintf(buffer,5,"%d",Player::stage+1);
    string strF(buffer);
