...
Enemy::Enemy(int enemyType,float life,nTPoint spawn,nTPoint size,vector<vector<GLuint> > animations,bool isHuman) {
    ...
    if(Player::stage>=0&&Player::stage<nicks.size())
...
nTPoint Enemy::defaultSize=Util::nTPointSet(16,48,0);
nTPoint Enemy::bossSize=Util::nTPointSet(48,100,0);
int Enemy::bossLife=30;
...
void Enemy::behave(){
    ...
    for(int i=0;i<Entity::enemys.size();i++){
        ...
        if(en->isVisible){
            GL::drawTexture(Util::nTRectangleSet(en->pos.x,en->pos.y+20,en->pos.x+20,en->pos.y,0.7,0.7),GL::getColorByName("white"),en->lifeLetter(),1);
            GL::setFontByIndex(4);
            GL::drawText(Util::nTPointSet(en->pos.x,en->pos.y-en->size.x-5,0.7),(char*)en->nickname.c_str(),GL::getColorByName("black"));
...
void Enemy::registerNicks(){
    ...
    tmp.push_back("Sistemas Operacionais de Redes");
    ...
    tmp.push_back("Banco de Dados");
    tmp.push_back("Modelagem e Desenvolvimento de Software");
    ...
    tmp.push_back("Interacao Humano Computador");
    tmp.push_back("Contexto Social e Profissional da Eng. de Comp.");
    nicks.push_back(tmp);
    tmp.clear();
