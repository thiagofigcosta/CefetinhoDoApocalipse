...
void Mechanics::move(int dir,float steeps){
    ...
    if(dir==Util::left||dir==Util::right){
      ...
      for(int i=0; i<var.size(); i++){
        ...
        if(var[i].collision.firstObj==2&&!(type>375&&type<=400)&&!(type==666)&&!(type>4000&&type<4013)&&!(type<-200))
            condition=false;
      }
      if(condition)
        pos.y+=steeps;
    ...
    if(c3 || c4){
      ...
      dist2= pos.x - (var[0].x+Blocks::defaultBlockSize.x/2);
        if(c1==true && c3==true){
          ...
          else if(dist-(r1.p0.y-r1.p1.y)/2<0 && dist2-Blocks::defaultBlockSize.x/2>=0){
            adc.firstObj=3;
            adc.secondObj=4;
          }
          else if(dist-(r1.p0.y-r1.p1.y)/2<=0 && dist2+Blocks::defaultBlockSize.x/2<0){
          ...
          else if(dist-(r1.p0.y-r1.p1.y)/2>=0){
            adc.firstObj=1;
            adc.secondObj=2;
          }
    }
