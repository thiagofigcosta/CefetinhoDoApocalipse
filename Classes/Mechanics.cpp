...
void Mechanics::move(int dir,float steeps){
    ...
    if(dir==Util::left||dir==Util::right){
      ...
      for(int i=0; i<var.size(); i++){
        ...
        if(var[i].collision.firstObj>=3&&!(type>375&&type<=400)&&!(type==666)&&!(type>4000&&type<4013))
          condition=false;
      }
      if(condition)
        pos.x+=steeps;
      ...
      for(int i=0; i<var.size(); i++){
        ...
        if(var[i].collision.firstObj==2&&!(type>375&&type<=400)&&!(type==666)&&!(type>4000&&type<4013))
                condition=false;
      }
      if(condition)
        pos.y+=steeps;
...
bool Mechanics::checkNormalForce(nTPoint pos_,nTPoint size_){
    ...
    nTPoint tmp;
    tmp.setPoint(pos_.x,pos_.y,pos_.z);
    ...
    for(int i=0; i<var.size(); i++){
        ...
        if(var[i].collision.firstObj==1&&!(type>375&&type<=400)&&!(type==666)&&!(type>4000&&type<4013)){
          condition=true;
        }
//        cout<<var[i].collision.firstObj<<"\n";
      }
//      if(condition)
//        cout<<"CHAO\n";
//      else
    ...
    if(c3 || c4){
      ...
      //cout<<"dist = "<<dist<<" dist2 = "<<dist2<<"\n";
        if(c1==true && c3==true){
            ...
            if(dist-(r1.p0.y-r1.p1.y)/2-10<0){
              adc.firstObj=2;
              adc.secondObj=1;
          }
            else{
          ...
          if(var[0].y-pos.y-(r1.p0.y-r1.p1.y)/2<2 && ABS(dist2)<Blocks::defaultBlockSize.x/2+(r1.p1.x-r1.p0.x)/2-4){
            adc.firstObj=2;
            adc.secondObj=1;
          }
          else if(dist-(r1.p0.y-r1.p1.y)/2-10<0 && dist2-Blocks::defaultBlockSize.x/2>=0){
            adc.firstObj=3;
            adc.secondObj=4;
          }
          else if(dist-(r1.p0.y-r1.p1.y)/2-10<=0 && dist2+Blocks::defaultBlockSize.x/2<0){
          ...
          else if(dist-(r1.p0.y-r1.p1.y)/2-10>=0){
            adc.firstObj=1;
            adc.secondObj=2;
          }
