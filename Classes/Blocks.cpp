...
void Blocks::move(int dir,float steeps){
    ...
    if(dir==Util::left||dir==Util::right){
      ...
      if(!condition){
        ...
        condition=false;
        teste=Mechanics::getCollision(Util::getCollisionRectangle(pos,size),Util::getCollisionRectangle(player->pos,player->size));
      if(teste.firstObj==2){
        nTPoint point;
      ...
      else if((((pos.x-player->pos.x<32)) || (player->pos.x-pos.x<32)) && fabs(player->pos.y-pos.y)<32){
        player->hSpeed=0;
      }
    }
