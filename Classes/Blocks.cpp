...
string Blocks::getTexNameByIndex(int BlockType){
    ...
    if(BlockType>=476&&BlockType<=500){
        return string("Bau");
    }else if(BlockType>=301&&BlockType<=325){
...
void Blocks::draw(){
    ...
    if(type>325&&type<=350){
        ...
        string bID(buffer);
        GL::drawTexture(Util::getCollisionRectangle(tmp,size),Util::nTColorSet(1,1,1,1),GL::getTextureByName("Quebrado"+bID),1);
        if(brokeStage>=10)
            delete this;
    }
}
