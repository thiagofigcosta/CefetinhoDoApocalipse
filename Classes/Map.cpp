...
void Map::refresh(){
    ...
    for(int i=0;i<dynamicBlocks.size();i++){
        ...
        if(bl->type>=101&&bl->type<=200){
            if(round(fmod(GL::framesInGame*GL::getMs(),1000/Bullet::timeToShoot))==0){
                nTPoint tmp=bl->pos;
        ...
        if(bl->type>=476&&bl->type<500&&player->atacking){
            objCollision var;
