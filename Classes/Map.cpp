...
void Map::refresh(){
    ...
    for(int i=0;i<dynamicBlocks.size();i++){
        ...
        if(bl->type>250&&bl->type<=300)
            ...
            if(round(fmodl(GL::getGameMs(),(int)Bullet::timeToShoot/5))==0){
                nTPoint tmp=bl->pos;
                tmp.z=0.9;
            ...
            if(round(fmodl(GL::getGameMs(),(int)Bullet::timeToShoot))==0){
                nTPoint tmp=bl->pos;
                tmp.z=0.9;
        ...
        for(int j=0;j<currentMap[0].size();j++){
            ...
            if(Blocks::checkIfBlocksIsDynamic(currentMap[i][j])){
                if(currentMap[i][j]==1000){
                    ...
                    bl->pos.z=0.89;
                    nOfEnemys++;
                    en=new Enemy(currentMap[i][j]-2000,Enemy::defaultLife,bl->pos,Enemy::defaultSize,Entity::getAnimationVector(enemyAnim[0],enemyAnimSize[0]),0);
...
void Map::next(){
    if(Scenes::freeGameMode){
        //TODO: perguntar pra onde vai(se vai continuar)
