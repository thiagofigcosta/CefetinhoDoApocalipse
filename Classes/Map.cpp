        ...
        if(bl->type>250&&bl->type<=300)
            ...
            if(round(fmodl(GL::getGameMs(),(int)Bullet::timeToShoot))==0&&player->life>0){
                ...
                if(player->pos.x>=bl->pos.x){
                    tmp.x+=Blocks::defaultBlockSize.x*1.2;
                    bu=new Bullet(4,Bullet::baseSpeed*1.7,tmp,Util::nTPointSet(100,60,1));
                }else{
            ...
            objCollision var;
            var=Mechanics::getCollision(Util::getCollisionRectangle(player->pos,player->size),Util::getCollisionRectangle(bl->pos,bl->size));
